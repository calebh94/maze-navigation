function opt_route = aStar_long_route(input_map, state, goal, map_struct, padding_range)
    ...
    for i = 1 : 8
        ...
        if (in_open == 0) && (in_close == 0)
            ...
            if map_status < 1
                ...
                top_status = updated_map(y_n-1, x_n);
                btm_status = updated_map(y_n+1, x_n);
                lft_status = updated_map(y_n, x_n-1);
                rgt_status = updated_map(y_n, x_n+1);
                
                if top_status < 1 && lft_status < 1
                    continue
                elseif top_status < 1 && rgt_status < 1
                    continue
                elseif btm_status < 1 && lft_status < 1
                    continue
                elseif btm_status < 1 && rgt_status < 1
                    continue
                elseif top_status < 1 && btm_status < 1
                    continue
                elseif lft_status < 1 && rgt_status < 1
                    continue
                else
